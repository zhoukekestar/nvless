...
const UUID = validate(process.env.UUID ?? '')
  ? process.env.UUID
  : v5(process.env.UUID ?? 'inputyouruuid', Buffer.alloc(16))
...
function mux (socket: WebSocket) {
  ...
  if (offset + metaLength > buffer.length) {
    //没有收全
  ...
  if (hasExtra && offset + extra_length > buffer.length) {
    //没有收全
    socket.pendings.push(buffer)
    return
  }
  ...
  const extra = hasExtra
    ? buffer.subarray(offset, (offset += extra_length))
    : undefined
...
function muxNewTcp (socket: WebSocket, session: MuxSession, meta: Buffer) {
  const target = createConnection(
    { host: session.dest.host, port: session.dest.port },
    () => {
      console.log(
        'mux tcp connected',
        session.id,
